...
def _bonus_request_queryset(request):
    return BonusRequest.objects.filter(
        Q(user__parent=request.user) | Q(user__parent__parent=request.user)
    ).select_related('user', 'bonus_rule', 'processed_by').order_by('-created_at')
...
def bonus_request_list(request):
    ...
    if err:
        return err
    qs = _bonus_request_queryset(request)
...
def bonus_request_detail(request, pk):
    ...
    if err:
        return err
    obj = _bonus_request_queryset(request).filter(pk=pk).first()
    if not obj:
...
def bonus_request_approve(request, pk):
    ...
    br = _bonus_request_queryset(request).filter(pk=pk, status='pending').first()
    if not br:
