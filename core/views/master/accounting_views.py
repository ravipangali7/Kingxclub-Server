"""Accounting report for master: date-scoped summary, game logs, transactions, deposits, withdrawals."""
...
def accounting_report(request):
    ...
    # Scope: master's players only (user__parent=request.user)
    base_user_filter = Q(user__parent=request.user)

    # Game logs in range
    game_logs_qs = (
        GameLog.objects.filter(base_user_filter)
    ...
    # Transactions in range (master's players or self)
    tx_qs = (
        Transaction.objects.filter(
            base_user_filter | Q(user=request.user)
        )
        .filter(created_at__date__gte=date_from, created_at__date__lte=date_to)
        .select_related("user")
        .order_by("-created_at")
    ...
    # Deposits in range
    dep_qs = (
        Deposit.objects.filter(base_user_filter)
        .filter(created_at__date__gte=date_from, created_at__date__lte=date_to)
    ...
    wd_qs = (
        Withdraw.objects.filter(base_user_filter)
        .filter(created_at__date__gte=date_from, created_at__date__lte=date_to)
        .select_related("user", "payment_mode")
        .order_by("-created_at")
    )
