"""Powerhouse: List all payment modes (pending), approve/reject."""
...
def payment_mode_verification_list(request):
    ...
    qs = PaymentMode.objects.all().select_related('user', 'action_by', 'payment_method').order_by('-created_at')
...
def payment_mode_approve(request, pk):
    ...
    if err:
        return err
    obj = PaymentMode.objects.filter(pk=pk, status='pending').first()
    if not obj:
