...
def bonus_request_list(request):
    ...
    if err:
        return err
    qs = BonusRequest.objects.all().select_related('user', 'bonus_rule', 'processed_by').order_by('-created_at')
...
def bonus_request_detail(request, pk):
    ...
    if err:
        return err
    obj = BonusRequest.objects.filter(pk=pk).select_related('user', 'bonus_rule', 'processed_by').first()
    if not obj:
...
def bonus_request_approve(request, pk):
    ...
    br = BonusRequest.objects.filter(pk=pk, status='pending').first()
    if not br:
